...
(function () {
  ...
  var operationsPages = ['console-activities.html', 'console-payments.html', 'console-collection.html'];
  ...
  var navItems = [
    ...
    { label: 'Business Operations', href: 'console-activities.html', icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
    { label: 'Tasks', href: 'console-tasks.html', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
  ...
  navItems.forEach(function (item) {
    var active = page === item.href || (item.href === 'console-activities.html' && operationsPages.indexOf(page) !== -1);
    html += '<a href="' + item.href + '" class="' + linkClasses(active) + '">';
    html += svg(item.icon);
    html += item.label;
    html += '</a>';

    if (item.href === 'console-activities.html' && operationsPages.indexOf(page) !== -1) {
      var operationChildren = [
        { label: 'Activities', href: 'console-activities.html' },
        { label: 'Payments', href: 'console-payments.html' },
        { label: 'Collections', href: 'console-collection.html' }
      ];
      html += '<div class="ml-8 mt-1 mb-2 space-y-0.5">';
      operationChildren.forEach(function (child) {
        var childActive = page === child.href;
        var childClass = childActive
          ? 'block px-3 py-1.5 text-sm font-medium text-everypay-700 bg-everypay-50 rounded-md'
          : 'block px-3 py-1.5 text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-50 rounded-md';
        html += '<a href="' + child.href + '" class="' + childClass + '">' + child.label + '</a>';
      });
      html += '</div>';
    }
  });
