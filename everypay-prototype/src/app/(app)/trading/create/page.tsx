...
export default function TradingActivityCreatePage() {
  ...
  const validate = (): boolean => {
    ...
    if (role === "buyer" && !sellerId.trim()) newErrors.sellerId = "Seller is required";
    if (role === "seller" && !buyerId.trim()) newErrors.buyerId = "Buyer is required";
  ...
  const handleSubmit = async (saveAsDraft: boolean) => {
    ...
    try {
      const res = await fetch("/api/procurements", {
        ...
        body: JSON.stringify({
          buyerId: role === "buyer" ? userId : buyerId,
          sellerId: role === "seller" ? userId : sellerId,
      ...
      if (result.status === "success" && result.data) {
        router.push(`/trading/${result.data.id}?userId=${userId}`);
      }
    } catch {
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <span className="text-gray-900 font-medium">New Activity</span>
      </div>
      ...
      <div className="flex justify-end space-x-3">
        ...
        >
          {submitting ? "Sending..." : "Send to Counterparty"}
        </button>
      </div>
    </div>
  );
